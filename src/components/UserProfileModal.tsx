...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({ isOpen, onClose, cpf }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            ...
            try {
                const result = await vendasService.obterPedidos(cpf);
                if (result.data) {
            ...
            {isOpen && (
                <>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            {/* Header minimalista */}
                            <div className="relative bg-gradient-to-br from-slate-50 to-slate-100 px-6 pt-6 pb-10">
                                <button
                                    onClick={onClose}
                                ...
                                <div className="flex flex-col items-center text-center">
                                    ...
                                    <h2 className="text-xl font-bold text-slate-900">
                                        {user ? user.name : "Meus Pedidos"}
                                    </h2>
                                    <p className="text-sm text-slate-500 mt-1">
                                        {user ? user.email : `CPF: ${cpf?.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4")}`}
                                    </p>
                                </div>
                            </div>

                            {/* Conteúdo */}
                            <div className={`px-6 pb-6 ${!user ? "pt-6" : ""}`}>
